...
/* proxy — routes peer / tracker / hostname lookups through HTTP or SOCKS5 */
#define LT_PROXY_NONE       0
#define LT_PROXY_SOCKS4     1
#define LT_PROXY_SOCKS5     2
#define LT_PROXY_SOCKS5_PW  3
#define LT_PROXY_HTTP       4
#define LT_PROXY_HTTP_PW    5

typedef struct {
    int32_t type;                 /* LT_PROXY_* */
    char    hostname[256];
    int32_t port;
    char    username[128];
    char    password[128];
    int32_t proxy_peer_connections;     /* 1 = route peers through proxy */
    int32_t proxy_tracker_connections;  /* 1 = route trackers through proxy */
    int32_t proxy_hostnames;            /* 1 = resolve hostnames via proxy */
} lt_proxy_config;

TORRENT_API void lt_set_proxy(lt_session_t session, const lt_proxy_config* config);
