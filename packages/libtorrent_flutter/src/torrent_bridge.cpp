...
TORRENT_API void lt_set_proxy(lt_session_t session, const lt_proxy_config* config) {
    ...
    if (!config || config->type == LT_PROXY_NONE || config->hostname[0] == '\0') {
        ...
        switch (config->type) {
            ...
            default:                 proxy_type = lt::settings_pack::none; break;
        }
        ...
        sp.set_bool(lt::settings_pack::proxy_peer_connections,
                    config->proxy_peer_connections != 0);
        sp.set_bool(lt::settings_pack::proxy_tracker_connections,
                    config->proxy_tracker_connections != 0);
        sp.set_bool(lt::settings_pack::proxy_hostnames,
                    config->proxy_hostnames != 0);
        // Prefer SOCKS5 UDP ASSOCIATE for DHT/uTP when available.
        ...
        // HTTP CONNECT cannot carry UDP; disable DHT/LSD/UPnP to avoid leaks.
