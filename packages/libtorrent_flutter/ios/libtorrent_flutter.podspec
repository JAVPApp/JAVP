Pod::Spec.new do |s|
  ...
  if !File.exist?(xcframework_absolute) && ENV['LIBTORRENT_FLUTTER_SKIP_DOWNLOAD'] != '1'
    ...
    if File.exist?(pubspec_path)
      ...
      if version_match
        # Upstream release tags are semver only (v1.9.2). JAVP's fork may use
        # a +build suffix (1.9.2+javp.1) which 404s if used in the download URL.
        plugin_version = version_match[1].split('+').first
