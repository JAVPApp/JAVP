...
class LibtorrentFlutter {
  ...
  /// Whether the loaded native library exposes [setProxy].
  bool get supportsProxy => _b.supportsProxy;

  /// Route peer / tracker / DNS traffic through [config], or clear the proxy
  /// when [LtProxyConfig.disabled] / inactive settings are passed.
  ///
  /// Returns `false` when the native library has no proxy API (unpatched
  /// upstream prebuilts). SOCKS5 is preferred for BitTorrent (UDP/DHT).
  bool setProxy(LtProxyConfig config) {
    final native = _b.setProxy;
    if (native == null) return false;
    final cfgPtr = calloc<LtProxyConfigNative>();
    try {
      final active = config.isActive;
      cfgPtr.ref.type =
          active ? config.type.nativeValue : LtProxyType.none.nativeValue;
      writeCharArray(
        cfgPtr.ref.hostname,
        256,
        active ? config.hostname.trim() : '',
      );
      cfgPtr.ref.port = active ? config.port : 0;
      writeCharArray(
        cfgPtr.ref.username,
        128,
      ...
      cfgPtr.ref.proxyPeerConnections = config.proxyPeerConnections ? 1 : 0;
      cfgPtr.ref.proxyTrackerConnections =
          config.proxyTrackerConnections ? 1 : 0;
      cfgPtr.ref.proxyHostnames = config.proxyHostnames ? 1 : 0;
      native(_session, cfgPtr);
      return true;
    } finally {
      calloc.free(cfgPtr);
    }
