...
// ─── Proxy ───────────────────────────────────────────────────────────────────
final class LtProxyConfigNative extends Struct {
  @Int32()
  external int type;
  @Array(256)
  external Array<Char> hostname;
  @Int32()
  external int port;
  @Array(128)
  external Array<Char> username;
  @Array(128)
  external Array<Char> password;
  @Int32()
  external int proxyPeerConnections;
  @Int32()
  external int proxyTrackerConnections;
  @Int32()
  external int proxyHostnames;
}

typedef _SetProxyN = Void Function(
    Pointer<LtSessionOpaque>, Pointer<LtProxyConfigNative>);
typedef LtSetProxy = void Function(
    Pointer<LtSessionOpaque>, Pointer<LtProxyConfigNative>);
...
void writeCharArray(Array<Char> arr, int maxLen, String value) {
  final units = utf8.encode(value);
  final n = units.length < maxLen - 1 ? units.length : maxLen - 1;
  for (var i = 0; i < n; i++) {
    arr[i] = units[i];
  }
  arr[n] = 0;
...
/// Optional JAVP proxy addon (ships next to the main lib on Android).
DynamicLibrary? _openProxyAddonLib() {
  try {
    if (Platform.isAndroid || Platform.isLinux) {
      return DynamicLibrary.open('libjavp_lt_proxy.so');
    }
  } catch (_) {}
  return null;
}

LtSetProxy? _resolveSetProxy(DynamicLibrary main, DynamicLibrary? addon) {
  if (main.providesSymbol('lt_set_proxy')) {
    return main
        .lookup<NativeFunction<_SetProxyN>>('lt_set_proxy')
        .asFunction<LtSetProxy>();
  }
  if (addon != null && addon.providesSymbol('lt_set_proxy')) {
    return addon
        .lookup<NativeFunction<_SetProxyN>>('lt_set_proxy')
        .asFunction<LtSetProxy>();
  }
...
class TorrentBridgeBindings {
  ...
  /// Null when the loaded native lib predates the proxy API.
  final LtSetProxy? setProxy;

  bool get supportsProxy => setProxy != null;

  TorrentBridgeBindings(this._lib)
      : setProxy = _resolveSetProxy(_lib, _openProxyAddonLib()) {
