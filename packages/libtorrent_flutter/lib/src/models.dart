...
/// Proxy type values matching native `LT_PROXY_*` constants.
enum LtProxyType {
  none(0),
  socks4(1),
  socks5(2),
  ...
  const LtProxyType(this.nativeValue);
  final int nativeValue;
}

/// Session-wide proxy for peer / tracker / DNS traffic.
class LtProxyConfig {
  const LtProxyConfig({
    this.type = LtProxyType.none,
    this.hostname = '',
    this.port = 0,
    this.username = '',
    this.password = '',
    this.proxyPeerConnections = true,
    this.proxyTrackerConnections = true,
    this.proxyHostnames = true,
  });

  static const disabled = LtProxyConfig();

  final LtProxyType type;
  final String hostname;
  final int port;
  final String username;
  final String password;
  final bool proxyPeerConnections;
  final bool proxyTrackerConnections;
  final bool proxyHostnames;

  bool get isActive =>
      type != LtProxyType.none && hostname.trim().isNotEmpty && port > 0;
}
